...
data {
  ...
  int ninterp;
  int nnorm;
  int ncomp; /* Number of components in GMM */

  real Tobs;
  int N_gen;

  vector[3] mu[nobs, ncomp];
  matrix[3,3] cov[nobs, ncomp];
  real wts[nobs, ncomp];
...
transformed data {
  real log_wtsel[nsel];
  real log_wts[nobs, ncomp];
  matrix[3,3] chol_cov[nobs, ncomp];
  ...
  for (i in 1:nobs) {
    for (j in 1:ncomp) {
      chol_cov[i,j] = cholesky_decompose(cov[i,j]);
      log_wts[i,j] = log(wts[i,j]);
    }
  }
}
...
parameters {
  ...
  real<lower=3, upper=100> m1[nobs];
  real<lower=0, upper=1> m2_frac[nobs];
  real<lower=0, upper=dl_max> dl[nobs];
}
...
model {
  ...
  /* A "population prior" for each event's parameters. */
  log_dN_nojac = log_dNdm1dm2ddldt_norm(m1, m2, dl, z, MMin, MMax, alpha, beta, gamma, dH, Om, w, sigma_low, sigma_high, ms_norm);

  /* Here we compute dN/d(m1)d(m2_frac)d(dL) = dN/d(m1)d(m2)d(dL) d(m2)/d(m2_frac) */
  for (i in 1:nobs) {
    log_dN[i] = log_dN_nojac[i] + log(m1[i]-3.0);
  }
  ...
  /* The GMM approximation to the posterior gets re-weighted to make the likelihood. */
  for (i in 1:nobs) {
    real logp[ncomp];
    /* We impose that the likelihood implements the symmetry m1 <--> m2 */
    vector[3] pt1 = to_vector({m1[i]*(1+z[i]), m2[i]*(1+z[i]), dl[i]});
    vector[3] pt2 = to_vector({m2[i]*(1+z[i]), m1[i]*(1+z[i]), dl[i]});

    for (j in 1:ncomp) {
      logp[j] = log_wts[i,j] + log_sum_exp(multi_normal_cholesky_lpdf(mu[i,j] | pt1, chol_cov[i,j]), multi_normal_cholesky_lpdf(mu[i,j] | pt2, chol_cov[i,j]));
    }
