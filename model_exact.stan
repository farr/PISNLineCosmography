...
transformed parameters {
...

  {
    ...
    for (i in 1:nobs) {
      ...
      m1source[i] = m1obs[i]/(1+zobs[i]);
      m2source[i] = m2obs[i]/(1+zobs[i]);
    }
...
model {
  ...
  log_pop_nojac = log_dNdm1dm2dzdt_norm(m1source, m2source, dlobs, zobs, MMin, MMax, smooth_min, smooth_max, alpha, beta, gamma, dH, Om, z_p, w0);
  /* We have dN/d(m1)d(m2)d(z)d(t_det) but data in m1obs, m2obs, so need
  /* d(ms)/d(mobs) = 1/(1+z) for each factor. */
  for (i in 1:nobs) {
    log_pop_jac[i] = log_pop_nojac[i] - 2.0*log1p(zobs[i]) + log(dzddL(dlobs[i], zobs[i], dH, Om, z_p, w0));
  }
